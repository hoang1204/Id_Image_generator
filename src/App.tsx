...
import { PromptSummary } from './components/PromptEditor'
import { PrintSheet } from './components/PrintSheet'
import { RestoreOptionControls } from './components/RestoreOptionControls'
import { RestoreSummary } from './components/RestoreSummary'
...
export default function App() {
  ...
  useEffect(() => { void photoIdApi.settings.get().then((s) => { setSettings(s); setChoices((current) => current.size === defaultChoices.size ? { ...current, size: s.imageSize } : current) }).catch((err: Error) => setError(err.message)) }, [])
  const updateChoices = (changes: Partial<EditorChoices>) => setChoices((current) => ({ ...current, ...changes }))
  const updateRestorationChoices = (changes: Partial<RestorationChoices>) => setRestorationChoices((current) => ({ ...current, ...changes }))
  ...
  const generate = async () => {
    ...
    try {
      const prompt = restoration ? buildRestorationPrompt(restorationChoices) : outdoor ? outdoorPrompt.trim() : buildPrompt(choices)
      if (!prompt) { setError('Lời nhắc chân dung ngoại cảnh không được để trống.'); return }
